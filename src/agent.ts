import type { ChatCompletionCreateParamsNonStreaming, ChatCompletionMessageParam, ChatCompletion } from "openai/resources/chat/completions";
...
async function withRetry<T>(operation: () => Promise<T>, options: RetryOptions = {}): Promise<T> {
  const {
    ...
    onRetry = (error: any, attempt: number) => console.log(`Retry attempt ${attempt} after error: ${error.message}`)
  } = options;
...
async function fetchWithRetry(url: string, options: FetchOptions = {}): Promise<import("node-fetch").Response> {
  const {
...
async function openaiWithRetry<T>(operation: () => Promise<T>, options: RetryOptions = {}): Promise<T> {
      ...
      try {
        ...
        const contentResponse = await fetchWithRetry(result.url, {
          ...
          retryOptions: {
            ...
            onRetry: (error: any, attempt: number) => console.log(`Retrying fetch for ${result.url}, attempt ${attempt}: ${error.message}`)
          }
        });
...
Please provide a comprehensive summary that synthesizes the information from these sources:`;
    ...
    const summaryResponse = await openaiWithRetry(() => 
      openai.chat.completions.create({
        ...
        onRetry: (error: any, attempt: number) => console.log(`Retrying OpenAI summary generation, attempt ${attempt}: ${error.message}`)
      }
    );
...
async function analyze_image({ path: imagePath, prompt = "Describe this image in detail" }: AnalyzeImageParams): Promise<string> {
  try {
    ...
    if (imagePath.startsWith('http://') || imagePath.startsWith('https://')) {
      ...
      try {
        ...
        const testResponse = await fetchWithRetry(imagePath, {
          ...
          retryOptions: {
            maxAttempts: 2,
            onRetry: (error: any, attempt: number) => console.log(`Retrying image URL check, attempt ${attempt}: ${error.message}`)
          }
        });
    ...
    const response = await openaiWithRetry(() =>
      openai.chat.completions.create({
        ...
        onRetry: (error: any, attempt: number) => console.log(`Retrying image analysis, attempt ${attempt}: ${error.message}`)
      }
    ) as ChatCompletion;
...
async function image_search_analysis({ query, analysis_prompt = "Describe and analyze these images in the context of the search query" }: ImageSearchAnalysisParams): Promise<string> {
  try {
    ...
    if (imageUrls.length === 0) {
      ...
      try {
        const altResponse = await fetchWithRetry(altSearchUrl, {
          ...
          retryOptions: {
            maxAttempts: 2,
            onRetry: (error: any, attempt: number) => console.log(`Retrying alternative image search, attempt ${attempt}: ${error.message}`)
          }
        });
        
        if (altResponse.ok) {
          const jsonData = await altResponse.json() as any;
          if (jsonData && jsonData.results) {
            jsonData.results.slice(0, 3).forEach((result: any) => {
              if (result.image) {
    ...
    for (let i = 0; i < Math.min(imageUrls.length, 3); i++) {
      ...
      if (!imageUrl) continue; // Skip if imageUrl is undefined
      ...
      try {
        ...
        const response = await openaiWithRetry(() =>
          openai.chat.completions.create({
            ...
            onRetry: (error: any, attempt: number) => console.log(`Retrying analysis for image ${i + 1}, attempt ${attempt}: ${error.message}`)
          }
        ) as ChatCompletion;
...
Please provide a comprehensive summary that identifies patterns, themes, and key insights across these images:`;
    ...
    const summaryResponse = await openaiWithRetry(() =>
      openai.chat.completions.create({
        ...
        onRetry: (error: any, attempt: number) => console.log(`Retrying combined summary generation, attempt ${attempt}: ${error.message}`)
      }
    );
...
Please provide an enhanced, more detailed version of the user request:`;
  ...
  const enhancementResponse = await openaiWithRetry(() =>
    openai.chat.completions.create({
      ...
      onRetry: (error: any, attempt: number) => console.log(`Retrying prompt enhancement, attempt ${attempt}: ${error.message}`)
    }
  );
  ...
  while (!isTaskComplete && turnCount < maxTurns) {
    ...
    const res = await openaiWithRetry(() =>
      openai.chat.completions.create({
        ...
        onRetry: (error: any, attempt: number) => console.log(`Retrying main agent call (turn ${turnCount}), attempt ${attempt}: ${error.message}`)
      }
    ) as ChatCompletion;
      ...
      if (!isTaskComplete) {
        const res2 = await openaiWithRetry(() =>
          openai.chat.completions.create({ 
            ...
            onRetry: (error: any, attempt: number) => console.log(`Retrying follow-up response (turn ${turnCount}), attempt ${attempt}: ${error.message}`)
          }
        ) as ChatCompletion;
